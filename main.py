...
parser.add_argument('--aa', type=str, default='augmix-m5-w4-d2', metavar='NAME',
...
def main():
    ...
    target_values = []
    for sample in dataset_train.parser.samples:
        if sample[1] in samples_per_class:
            target_values.append(sample[1])
            samples_per_class[sample[1]] += 1
        else:
        ...
        if args.token_label_size == 1:
            ...
            if args.ground_truth:
                train_loss_fn = TokenLabelGTCrossEntropy(dense_weight=args.dense_weight,\
                    cls_weight = args.cls_weight, mixup_active = mixup_active, weights=class_weights).cuda()
            else:
